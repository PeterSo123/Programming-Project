...
while($row=mysqli_fetch_array($search_result)){
    ...
    if( ((($row["wiring_date"] == NULL && $row["order_status"] == "Assigned") || $row['reason_id'] == 2) && number_of_working_days($row["pre_visit_date"],date("Y-m-d")) >=2)
        || ($row['reason_id'] == 3 && number_of_working_days($row["wiring_date"],date("Y-m-d")) >=2)
        ) {
    ...
    else if(($row['reason_id'] >= 14 && $row['reason_id'] <=22) && number_of_working_days($row["pre_visit_date"],date("Y-m-d")) >=$row['delay_date']){
    ...
    else if($row['reason_id'] == 23 && number_of_working_days($row["pre_visit_date"],date("Y-m-d")) >=$row['delay_date']){
